  ...
  if (testSpeedo) {
    ...
    if (dutyCycle != dutyCycleIncoming) {  // only update PWM IF speed has changed (can cause flicker otherwise)
      if (incomingType == 0) {             // if 'Can2Cluster'
        ...
        // build up the array to get the average
        if (rawCount < averageFilter - 1) {
          ...
          dutyCycle = findClosestMatch(dutyCycle);  // find the closest final duty based on the incoming duty (use motor perfomance)
      ...
      if (incomingType == 1) {  // if 'Hall'
        DEBUG_PRINTF("     DutyIncoming: %d", dutyCycleIncoming);
        dutyCycleIncoming = map(dutyCycleIncoming, minFreqHall, maxFreqHall, minSpeed, maxSpeed);  // map incoming range to this codes range.  Should match, but sense check...
...
uint8_t findClosestMatch(uint8_t val) {
  uint8_t closest = 0;
  for (uint8_t i = 0; i < sizeof motorPerformance / sizeof motorPerformance[0]; i++) {
